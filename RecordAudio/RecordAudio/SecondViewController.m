...
- (void)playAudioWithModel:(CustomCellModel *)model{
    ...
    [[JZJAudioPlayManager shareInstance] playWithPath:model.audioPath completion:^(JZJAudioPlayError error) {
        _previousSelectedModel.isPlaying = NO;
        [self.tableView reloadData];
...
- (IBAction)touchDown:(id)sender {
    [[JZJAudioRecordManager shareInstance] startRecordWithPath:@"test.aac" completion:^(JZJAudioRecordError error) {
        
    }];
    __weak SecondViewController *weakSelf = self;
    [JZJAudioRecordManager shareInstance].refreshVoiceBlock = ^(double voiceValue)
    {
        [weakSelf setVoiceImageWithValue:voiceValue];
    };
}

-(void)setVoiceImageWithValue:(double)value
{
    _recoredAnimationView.hidden = NO;
    double voiceSound = value;
...
- (IBAction)touchUpInside:(id)sender {
    NSLog(@"%s", __func__);
    [[JZJAudioRecordManager shareInstance] stopRecordWithCompletion:^(NSString *recordPath, JZJAudioRecordError error) {
        NSLog(@"error = %d",error);
        _recoredAnimationView.hidden = YES;
        if (error == JZJAudioRecordError_None) {
